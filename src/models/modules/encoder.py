...
class GatedBertEncoder(nn.Module):
    def __init__(self, manager):
        ...
        self.pooler_query = nn.Parameter(torch.randn((1, manager.plm_dim), requires_grad=True))
        nn.init.xavier_normal_(self.pooler_query)
        self.Tanh = nn.Tanh()
    ...
    def forward(self, token_id, attn_mask, token_weight=None):
        ...
        embedding = scaled_dp_attention(self.pooler_query, token_embedding, token_embedding, attn_mask=attn_mask.unsqueeze(-2)).squeeze(dim=-2).view(*original_shape[:-1], -1)
        return embedding
