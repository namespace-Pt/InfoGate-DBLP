...
class Manager():
    ...
    def __init__(self, config=None, notebook=False):
        ...
        parser.add_argument("-e", "--epoch", dest="epochs", help="epochs to train the model", type=int, default=50)
        ...
        parser.add_argument("-bs", "--batch-size", dest="batch_size", help="batch size in training", type=int, default=100)
        parser.add_argument("-bse", "--batch-size-eval", dest="batch_size_eval", help="batch size in evaluating", type=int, default=300)
        ...
        self.file_name = file_name_map[self.enable_gate]
        self.dataloaders = dataloader_map[self.mode]

        self.distributed = self.world_size > 1
        self.exclude_hparams = set(["metrics", "plm_dim", "plm_dir", "data_root", "cache_root", "distributed", "exclude_hparams", "rank", "epochs", "mode", "debug", "special_token_ids", "validate_step", "hold_step", "exclude_hparams", "device", "save_at_validate", "base_rank"])
    ...
    def save(self, model, step, best=False):
        ...
        if isinstance(model, torch.nn.parallel.DistributedDataParallel):
            model = model.module
        if best:
            save_path = f"data/ckpts/{model.name}/best.model"
        else:
            save_path = f"data/ckpts/{model.name}/{step}.model"
    ...
    def load(self, model):
        ...
        if checkpoint == "none":
            ...
            save_path = f"data/ckpts/{model.name}/{checkpoint}.model"
    ...
    def train(self, model, loaders):
        ...
        if self.rank == 0:
            ...
            os.makedirs(f"data/ckpts/{model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name}", exist_ok=True)
